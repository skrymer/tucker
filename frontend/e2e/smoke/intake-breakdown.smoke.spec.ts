...
// backend. Foods and Entries are seeded through the API, the section is asserted
// on screen, and everything seeded is removed again.
...
test("the day's calories are divided between the Foods they went on", async ({
  ...
  try {
    ...
    for (const grams of [120, 80]) {
      ...
      entryIds.push(((await logged.json()) as { id: number }).id)
    }
    ...
    entryIds.push(((await loggedRice.json()) as { id: number }).id)
    ...
    entryIds.push(((await canteen.json()) as { id: number }).id)
    ...
    for (const id of entryIds) await request.delete(`${API}/entries/${id}`)
    await request.delete(`${API}/foods/${chickenFood.id}`)
    await request.delete(`${API}/foods/${riceFood.id}`)
  }
})
