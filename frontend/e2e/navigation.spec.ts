...
test.describe('app shell navigation', () => {
  test('shows the bottom tab bar on a phone-width viewport', async ({ page, goto }) => {
  ...
  test('shows the side navigation on a desktop-width viewport', async ({ page, goto }) => {
  ...
  test('opens the selected destination while the shell persists', async ({ page, goto }) => {
    await page.setViewportSize(DESKTOP)
    await goto('/', { waitUntil: 'hydration' })
    ...
    await expect(page.getByRole('heading', { level: 1, name: 'Foods' })).toBeVisible()
