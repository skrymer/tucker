...
- **Progress / meter** — fully rounded track on `--ui-bg-muted`, fill in the
  ...
  **A meter is named for what it measures**, and that name is set with
  ...
  defaults it to the percentage. Nothing is lost by that: the figure is the
  _value_, and `aria-valuenow` with `aria-valuemax` carries it — on the Day Ring
  as `1004` of `2140`, from which assistive tech derives the percentage itself.
