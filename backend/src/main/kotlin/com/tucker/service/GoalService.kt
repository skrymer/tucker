...
class GoalService(
     ...
     *
     * The **start weight is derived here** as the **live** Trend Weight (ADR 0016)
     * — the client can't compute the EWMA, and anchoring on the trend makes a fresh
     * Goal read 0% (start == now). [startedOn] says when the plan runs from; it is
     * never what the anchor is looked up by. With no reading at all there is no
     * trend, so the Goal can't be anchored and is rejected.
     *
    ...
    /**
     * The target sits below the live Trend Weight — also the weight the Goal starts
     * from, the two being one figure — or the Goal is already reached (ADR 0016).
     * Stated here rather than left to [Goal]'s own `target < startWeight` invariant
     * so the refusal names `targetWeightKg`: a bare invariant failure carries no
     * field, and the form shows it above the submit as though no input were at fault.
     */
